...
alpha_indB = 0.0*1e-3; % attenuation (dB/km) --> dB/m
D_EDF = -5e-6; % GVD (ps/nm.km); if anomalous dispersion(for compensation),D is negative
n2_EDF = 1*2.6e-20; % nonlinear index (m^2/W)
Aeff_EDF = 35e-12; % effective area (m^2)
% Slope Dispersion
...
Beta3 = (beta3_SMF*Length_SMF+beta3_EDF*Length_EDF)/Length_total ;
...
Usol = sqrt(P0).*(r*sech(tstep./To-qo).*exp(-j*teta)+sech ...
(tstep./To-0).*exp(+j*teta)+r.*sech(r.*(tstep./To+qo)).*exp ...
(-j*teta)).*exp(-j*Ci*tstep.^2./2/To.^2) ;
% Usol = sqrt(P0).*(r*sech(tstep./To-1*qo).*exp(+j*teta)+r*sech(tstep./To-qo/3).*exp(-j*teta/1)...
% +sech(tstep./To+qo/3).*exp(+j*teta/1)+r.*sech(r.*(tstep./To+1*qo)).*exp(-j*teta)).*exp(-j*Ci*tstep.^2./2/To.^2) ;
% Eo = Usol;
...
for ii = 2:N_pass
	...
	[Usol,A3d,z]=hconst(Usol,Length_total,5,1*Beta2,1*Beta3,alpha_loss_SMF,1*Gamma,0); % Averaged Fiber
	% Eo = modInt(Eo(1:N),alpha,epsilon,m,fm,0.5);% MZI modulator
	if ii <= round(N_pass/2)
		Usol = pha_mod(Usol(1:Ns),Vm,Vbias,Vpi,fm,p0); % Phase modulator
		% Usol = Usol.*exp(-j*Ci*tstep.^2./2/To.^2);
		% Usol = hpha_mod(Usol(1:Ns),Vm,Vbias,Vpi,fm,2,0);
	end
	% Usol = Usol*atten;
...
dff = -diff(Pha)/Ts;
subplot(211);plot(tstep*1e12,Pha);xlabel('Time (ps)');ylabel('Voltage (V)');axis tight;
subplot(212);plot(tstep(1:Ns-1)*1e12,dff);xlabel('Time (ps)');ylabel('Phase (rad)');
