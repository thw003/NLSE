function Eout = filter_gaus_stark_shift(Ein,f3dB,shift,n)
...
% k = (1:N)-N/2
% T = (exp(-log(sqrt(2))*(2/f3dB/Ts/N)^2*((k-shift).^2))+exp(-log(sqrt(2))*(2/f3dB/Ts/N)^2*((k+shift).^2)))/2
...
Eout = Ein.*(exp(-temp*((k-shift).^n))+exp(-temp*((k+shift).^n)))/2;	% n order gaussian filter VPI
