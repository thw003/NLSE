...
% =======================================================
% Fiber parameters
Length_SMF = 65; % Length of a span in meter
nz = Length_SMF/100;
alpha_indB = 0.0*1e-3; % attenuation (dB/km) --> dB/m
...
% -------------------------------------------------------
gamma_SMF = 2*pi*n2_SMF/(lamda*Aeff_SMF); % nonlinearity coefficient (km^-1.W^-1)
% =======================================================
% filter bandwidth
...
Length_EDF = 50; % Length of a span in meter
alpha_indB = 0.0*1e-3; % attenuation (dB/km) --> dB/m
D_EDF = -7e-6; % GVD (ps/nm.km); if anomalous dispersion(for compensation),D is negative
...
Tfwhm = 5.4e-12;
To = Tfwhm/1.665; % Pulsewidth of gaussian
Tb = 24e-12; % Pulses distance
teta = 1*pi/2; % Relative phase difference between 2 solitons
r = 1; % Amplitude ratio
...
% Usol = sqrt(P0)*(exp(-(tstep-Tb/2).^2/2/To^2).*exp(-j*teta)+r*exp(-(tstep+Tb/2).^2/2/To^2).*exp(+j*teta)).*exp(-j*Ci*tstep.^2./2/To^2);%*exp(j*(2*pi*f*ti+phiC));
% Usol = sqrt(P0).*sech(tstep./To).*exp(-j*teta).*exp(-j*Ci*tstep.^2./2/To.^2);
% Usol = sqrt(P0).*(sech(tstep./To-qo)+r.*sech(r.*(tstep./To+qo)).*exp(j*teta)).*exp(-j*Ci*tstep.^2./2/To.^2) ; %
Usol = sqrt(P0).*(sech(tstep./To-qo).*exp(-j*teta)+r.*sech(r.*(tstep./To+qo)).*exp(+j*teta)).*exp(-j*Ci*tstep.^2./2/To.^2) ;
% Usol = sqrt(P0).*(r*sech(tstep./To-qo).*exp(+j*teta)+sech(tstep./To-0)+r.*sech(r.*(tstep./To+qo)).*exp(+j*teta)).*exp(-j*Ci*tstep.^2./2/To.^2) ;
% Eo = Usol;
...
Up(1,:) = Usol;
pause(0.1)
% ----- Algorithm for Soliton Beam Propagation Method : Split Step Fourier Model -----%
% for a = 2:1:Block
% for b = 1:BlockSize
%
% D = -j/2*Beta2.*(j.*w).^2 + 1/6*Beta3.*(j.*w).^3 - Alpha/2 ;
% N1 = j*Gamma.*(abs(Usol)).^2 ; %-2*Gamma*OpLamda./(2*pi*c.*U1).*(j.*w.*(abs(U1).^2.*U1)) ;
% N2 = N1 ;
%
% % First half interval : dispersion operator only
% Udisp = ifft(exp(h/2.*D).* fft(Usol)) ;
%
% % Iteration process to find N2
% for c = 1:2
% Utmp = ifft( exp(h/2.*D).* fft(Usol)) ;
% UnonTmp = exp(h/2.*(N1+N2)).* Utmp ;
% U2tmp = ifft(exp(h/2.*D).* fft(UnonTmp)) ;
% N2 = j*Gamma.*(abs(U2tmp)).^2 ;
% end
%
% % Second half interval : non linear operator only (lumped)
% Unon = exp(h/2.*(N1+N2)).* Udisp ;
%
% % Whole segment propagation
% U2 = ifft(exp(h/2.*D).*fft(Unon));
% Usol=U2;
% end %b
%
% ARR2(a,:)=abs(U2).^2 ;
%
% plot1Hndl=plot(t*1e12, abs(U2).^2) ;grid; xlabel('Time(ps)');
% ylabel('P (W)');
% zz = a*BlockSize*h/1000;
% global b0
% str0=num2str(zz);
...
% 				'FontSize',10, ...
% 				'String',['z = ',str0,' km'],'Position',[500 310 100 15]);
%
% %axes(axHndl1);
% pause(0.5)
% end
...
for ii = 2:N_pass
	...
	% [Eo,A3d,z]=hconst(Eo,Length_SMF,5,1*beta2_SMF,1*beta3_SMF,alpha_loss_SMF,1*gamma_SMF,0); % Fiber section #2
	...
	% Usol = hpha_mod(Usol(1:Ns),Vm,Vbias,Vpi,fm,3,p0);
	% Usol = Usol*atten;
	...
	% Up(ii,:) = Usol;
	% plot1Hndl=plot(tstep*1e12, abs(Usol).^2) ;grid; xlabel('Time(ps)');
	% ylabel('P (W)');
	% -----------------------------------------------------

	% Display the number of round trips
	% global b0
	% str0 = num2str(ii);
	% b0 = uicontrol('Parent',h0,...
	%				'Style','text','BackgroundColor',[1 1 1], ...
	...
	% 				'ForegroundColor','blue',...
	% 				'FontSize',10, ...
	% 				'String',['roundtrips = ',str0,'rounds'],
	% 				'Position',[500 310 100 15]);
	% % -----------------------------------------------------
	% pause(0.01)
	% % close(h1);
end
