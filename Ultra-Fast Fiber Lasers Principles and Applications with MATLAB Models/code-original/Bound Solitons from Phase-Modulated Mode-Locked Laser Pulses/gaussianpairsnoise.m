...
% =======================================================
% Fiber parameters
Length_SMF = 95; % Length of a span in meter
nz = Length_SMF/100;
...
Length_EDF = 20; % Length of a span in meter
alpha_indB = 0.0*1e-3; % attenuation (dB/km) --> dB/m
...
Usol = sqrt(P0)*exp(-tstep.^2/2/To^2).*exp(-j*Ci*tstep.^2./2/To^2);%*exp(j*(2*pi*f*ti+phiC));
% Eo = Usol;
...
% plot (tstep*1e12,Pha_tinput);grid;
plot (tstep(1:Ns-1)*1e12,ff);grid;
% plot (tstep(2:Ns-1)*1e12,chrate);grid;392	
PP = angle(Up(NN,:));
Pha_tinput = unwrap(PP);
ff = -diff(Pha_tinput)/Ts;
chrate = diff(ff)/Ts;
